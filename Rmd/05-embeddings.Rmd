...
workflow steps rely on the embedding being representative of the original high
...
embeddings and performing cluster orientation tasks. Several
...
an analysts workflow. Instead, we implement a more pragmatic workflow by
...
can be easily incorporated into an analysts workflow with our interface in 
**liminal**. 
We have made extensive use of ensemble graphics, that is
...
are identified from linked brushing on the tour view, higlighting points
in the embedding view. Section <> details 


## Software Infrastructure

We have implemented the above design as an open source R package called
**liminal**. The package allows analysts to construct concatenated
...
view. For interpreting specific combinations of clusters, the legend
...
where t-SNE is applied to  simulations where cluster structure and geometry of the underlying data is known.  
...
In future implementations it will be possible to shift from one to one linked
brushing to one to many linked brushing. This form of brushing acts directly on
a $k$-nearest neighbours ($k$-nn) graph computed from a
...
can be independent of the tour or embedding views. The estimation of neighbours
can be computed effeciently with the `FNN` or `RcppAnnoy` packages
on the server side [@fnn-pkg; @rcpp-annoy-pkg].. In place of highlighting, one 
could use opacity or binned colour scales to encode distances or ranks instead 
of the neighbouring points, Brush composition, such as 'and', 'or',
or 'not' brushes, could also be used to interrogate mismatches between the 
$k$-nn graphs estimated from both the embedding and tour views.
