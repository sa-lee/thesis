...
Non-linear methods generally perform preprocessing on the high-dimensional data
such as generating a neighborhood graph and perform transformations on the
preprocessed form. We restrict our attention to three methods that are commonly
...
the one that minimizes the Kulback-Leibler divergence between the two
...
parameterised [@Wattenberg2016-ji; @Kobak2019-lm]. UMAP is a method that is
...
defines a different kernel for similiarites in the low dimensional space. Due
...
graph. The algorithm denoises estimated distances in high dimensional space via
transforming the affinity matrix into a Markov transition probaility matrix and
...
2D scatter plots were mostly suffucient for detecting class seperation, however
...
to interact with these techniques and assist them in making comparsions between
...
methods, there are a lot of parameters to explore that can have an effeft on
...
first run principal components on the input as an intial dimension reduction
...
data onto hyperdimensional unit cube. We bind the half range to a mouse wheel
...
We combine the tour view in a side by side layout with a scatter
plot view from an embedding algorithm. The views are linked; analysts can brush a group of points in the embedding view, and see its shape in the tour view.
This can be used for either manually identifying clusers, or verifying the location of clusters relative to other groupings. Consequently, simple linked
brushing can alleviate a downfall of non-linear methods such as UMAP or t-SNE
that break hierachial structure of clusterings. Aside from simple
...
single point highlights or colorings, allowing analysts to get a more holistic
...
### Linking views via spatial brushes
