```{r setup-videos, cache = FALSE}
vid_lookup <- tibble::tribble(
  ~key, ~url, ~path,
  "gaussian",  "https://player.vimeo.com/video/439635921", here::here("video", "liminal-gaussian.mp4"),
  "hierachical", "https://player.vimeo.com/video/439635905", here::here("video", "liminal-hierachical.mp4"),
  "trees-01", "https://player.vimeo.com/video/439635892", here::here("video", "liminal-trees-01.mp4"),
  "trees-02", "https://player.vimeo.com/video/439635863", here::here("video", "liminal-trees-02.mp4"),
  "mouse-01", "https://player.vimeo.com/video/439635812", here::here("video", "liminal-mouse-01.mp4"),
  "mouse-02", "https://player.vimeo.com/video/439635843", here::here("video", "liminal-mouse-02.mp4")
)
...
display_vid_latex <- function(key, lookup) {
    video <- lookup[lookup$key == key, ]
    glue::glue(
    "\\begin{{center}}
   \\includemedia[
...
# Casting multiple shadows: high-dimensional interactive data visualisation with tours and embeddings {#ch:tsne}
...
single cell transcriptomics and image processing, that shows our approach is
...
workflow steps rely on the embedding being representative the original high
dimensional dataset, and becomes much more difficult when there is no underlying
ground truth.
...
animated visualization.Given the dynamic nature of the tour, user interaction is 
important for controlling and exploring the visualisation: the tour has been
used previously by @Wickham2015-cx and @Li2020-rh as a tool for 
exploring statistical model fits and by @Buja1996-fk for exploring the space of 
...
visualisation for colouring points in an embedding according to an estimated
pair-wise distance in the original high-dimensional space [@Ovchinnikova2020-sy].
The idea behind this is that non-linear embedding methods should preserve 
locality and that points that are 'close' to each other on the embedded view
should be close according to the estimated pair-wise distance. 
Similarly, the TensorFlow embedding projector is a web interface to 
...
guided and modified form of the  t-SNE algorithm, that allows users to modified 
...
approach is the PHATE algorithm [@Moon2019-ce]. Here an affinity matrix is 
estimated via the pair-wise distance matrix and k-nearest neighbours graph of $X$. 
...
and classical MDS is  used to generate $Y$. A general difficulty with using 
...
we now examine the underpinnings of t-SNE and UMAP.
...
Next, the target low-dimensional space, $Y$, pair-wise distances between
...
pair-wise distances in $Y$ are large regardless of the magnitude of the
...
supposedly preserves global structure better than t-SNE [@Coenen2019-to].
...
aligning related plots alongside each other to provide context [@Unwin2018-zb]. 
As we will see in the case studies, this allows analysts to quickly compare 
views from embedding methods and allows them to see how the embedding method 
alters the global structure of their data. Using ensembles allows the use of 
interaction techniques, that allow analysts to perform cluster orientation 
tasks via linking multiple views. This approach allows our interface, to achieve 
...
We propose this can be
done visually using our side-by-side tour and embedding views.
In the simplest case, a local continuity check can be assessed via one to
one linked brushing from the embedding to the tour view. This can be generalised further with one to many brushing, via spatial brushing. This form
of brushing acts directly on $k$-nearest neighbours ($k$-nn) graph computed from a
...
corresponding tour view. In place of highlighting, one could use opacity
or binned colour scales to encode distances or ranks instead of the 
neighbouring points, or by highlighting. Brush composition, such as 'and', 'or', or 'not' brushes,  could also be used to interrogate mismatches between the $k$-nn graphs estimated from both the embedding and tour views. This is left for 
future implementations of **liminal**.
...
visualisations, via the Vega-Lite grammar of interactive graphics via
the **vegawidget** package [@Satyanarayan2017-gs; @Lyttle2020-hp]. It provides an interface for constructing linked
and stand alone interfaces for manipulating tour paths via the **shiny** and **tourr** [@Chang2020-bq; @Wickham2011-st]. Below we provide some background
on our implementation of the tour and user interaction.


### Tours explore the subspace of $d$-dimensional projections 

Here we provide an overview of the mathematical aspects of the tour for
readers who are unfamiliar with the technique before discussing the 
computational aspects of how **liminal** interfaces with them. The tour
is emphatically not a dimension reduction technique but rather a visualisation 
technique that is grounded in mathematical theory, that is able to ascertain 
the shape and global structure of a dataset via inspection of the subspace 
generated by the set of low-dimensional projections. In fact, in some
of the case studies below, we  first sphere our data via principal 
components to reduce dimensionality of our input. 
...
$A_{t \in \mathbb{N}}$, where $d$ is typically $1$ or $2$. For each pair of 
...
continuous visualisation of the  projections $Y_{t} = XA_{t}$, that is the 
projection of $X$ onto $A_{t}$ as the tour path is interpolated between 
successive bases. A *grand tour* corresponds to choosing new orthonormal
bases at random; allowing a user to ascertain structure via exploring the
subspace of $d$-dimensional projections. Instead of picking projections at 
random, a *guided tour* can be used to generate a sequence 'interesting' 
projections as quantified by an index function. While **liminal** is able to 
visualise guided tours, our focus in the case studies uses the grand tour to
see global structure in the data. 
...
technique. Generally, the user would set $d=2$ the tour is visualised as
a animated scatter plot. This process of constructing bases and intermediate 
...
view (the special case of $k=1$ nearest neighbours brush). In future
implementations it will be possible to shift from one-to-many linking by increasing $k$ via the 'controls' panel of the interface.  If desired, the distance metric used to estimate nearest neighbours can by chosen by the user. 
The user may provide a reference data set for computing
nearest neighbours (for example a distance matrix, or the complete data matrix)
independently of the tour or embedding views. The estimation of neighbours
is done with the `FNN` package for datasets with less than 4096 observations
and `RcppAnnoy` for larger datasets or if a user selects a non-Euclidean
distance metric [@fnn-pkg; @rcpp-annoy-pkg]. Once the user is finished 
with the interface, the brush regions are returned for downstream analysis.


## Case Studies

The next section steps through case studies cases of our approach using simulated
and real data.  The videos presented show the visual 
appearance of the **liminal** interface, and how we can interact with the tour
via the controls previously described.
In the first case study, we look at three different examples
where t-SNE is applied to three simulations where cluster structure and geometry
of the underlying data is known.  

In the second case study, we apply our approach to clustering 
the mouse retina data from @Macosko2015-ot, and use apply the tour
to the process of verifying marker genes. 

<!--
Finally, in the last case study we
...
### Case Study 1: Exploring simulation data with tours and t-SNE
...
using the **Rtsne** package [@Rtsne],
and set up the **liminal** linked interface with grand tour on the 10-$d$
observations: 


```{r gaussian, results="asis", eval = knitr::is_latex_output()}
cat(display_vid_latex("gaussian", vid_lookup))
```
...
We see in the video that t-SNE has correctly split out each cluster and
laid them out in a star like formation. This is agrees with the tour view,
...
the t-SNE layout. This can be seen from our brushing action on the t-SNE side. 

We repeat the process of simulating Gaussian clusters in the next video, 
but this time we have a cluster that has more granular structure. This time we have two
Gaussian clusters with equal covariance embedded in $10-d$, and a third cluster
with hierarchical structure. This cluster has two 3-$d$ clusters embedded in 
10-$d$, where the second cluster is subdivided into three smaller clusters, that are each equidistant from each other and have the same covariance structure.
...
far from each other in the tour view. We see in this case that is difficult
to see the sub-clusters in the tour view, however, once we zoom and highlight
they become more apparent. When we brush the sub-clusters in the t-SNE, their
...
```{r hierachical, results='asis', eval = knitr::is_latex_output()}
cat(display_vid_latex("hierachical", vid_lookup))
```

```{r hierachical-h, eval = knitr::is_html_output()}
display_vid_html("hierachical", vid_lookup)
```


Next explore some simulated noisy tree structured data (figure
...
(ref:fake-trees) Example high-dimensional tree shaped data, $n = 3000$ and $p = 100$. *(a)* The true data lies 2-d tree like structure consisting of ten branches. This data is available in the **phateR** package and is simulated via diffusion-limited aggregation (a random walk along the branches of the tree) with Gaussian noise added [@Moon2019-ce]. *(b)* The first two principal components, which form the initial projection for the tour, note that the backbone of the three is obscured by this view. *(c)* The default t-SNE view breaks the global structure of the tree. *(d)* Altering t-SNE using the first two principal components as the starting coordinates for the embedding, results in clustering the tree at its branching points.

```{r fake-trees, echo = FALSE, fig.align = "center", fig.cap="(ref:fake-trees)"}
knitr::include_graphics(here::here("img", "tree-view.png"))
```
...
```{r trees-01, results='asis', eval = knitr::is_latex_output()}
cat(display_vid_latex("trees-01", vid_lookup))
```
...
```{r trees-02, results='asis', eval = knitr::is_latex_output()}
cat(display_vid_latex("trees-02", vid_lookup))
```
...
the branches (the three black dots in figure \@ref(fig:fake-trees) a) being split
into three clusters representing the terminal branches of the tree. However, there
...
and look at marker genes through the lens of the tour.
...
(described by @Amezquita2020-at): we performed QC using the the `scater`
...
To check and verify the clustering we construct a **liminal** view. 
...
get a sense of the shape of the clusters as seen in the video below:


```{r mouse-01, results='asis', eval = knitr::is_latex_output()}
cat(display_vid_latex("mouse-01", vid_lookup))
```
...
In the following video we can observe that the embedding has mostly captured the
...
linked brushing. As expected avoids t-SNE mitigates the crowding problem that is
an issue for tour in this case, where points in clusters 2,4,6, and 11 are
clumped together in tour view, but are blown up in the embedding view. 
The tour appears to form a tetrahedron like shape, with points lying on
surface and along the vertices of the the tetrahedron in 5-$d$ PCA space -
a phenomena that has also been observed in @Korem2015-af. Brushing on
the tour view, reveals points in cluster 9 that are diffuse in the embedding 
view, points in cluster 9 are relatively far away and spread apart from other clusters in the tour view, but has points placed in cluster 3 and 9 in the embedding. 
...
uses the testing framework from @McCarthy2009-qx. We select the top 10 marker genes that are upregulated in cluster 2, which was one of the clumped clusters
...
```{r mouse-02, results='asis', eval = knitr::is_latex_output()}
cat(display_vid_latex("mouse-02", vid_lookup))
```
...
We see that the expression of the marker genes using t, appear to separate
the previously clumped clusters 2,4,6, and 11 from the other clusters, 
indicating that these genes are expressed in all four clusters, and the 
magnitude of expression in these genes is similar across these groups, 
consistent with the results of marker gene analysis.


<!--
### Case Study 3: How do people draw animals?

In the final case study, we explore a subset of the Google QuickDraw dataset 
[@quickdraw-paper]. The data are sketches drawn by people of common concepts
...
We downloaded the metadata and normalised 28 by 28 pixel bitmaps via the 
`quickdraw` package in R; unravelling the bitmaps unravelled the bitmaps into 
a 784 dimension image vector. We sampled 500 images from each of the following
categories: "broccoli", "flying saucer", "ice cream", "octopus", "pineapple",
and "whale". We are interested in discovering images that are similiar but
belong to different classifications 
-->
...
generated from embeddings. The interface we have designed in the **liminal**
...
studies, the t-SNE method can produce misleading embeddings but can be
...
Our holistic framework is also useful in real data situations, as displayed
in our second case study. The tour when combined with t-SNE allowed us
...
in using the **liminal** interface for larger datasets. First,
t-SNE avoids the crowding problem, points are separated into distinct regions
on the display canvas, while for the tour, points are concentrated in the
centre of the projection and become difficult to see. 
Second, as $n$ increases the animation becomes slower as there is more data
passing from the server to the client. Both of these issues could be mitigated
through alternate tour paths and displays. The recent work
...
We would like to thank David Frazier, Ursula Laa and Paul Harrison for their
