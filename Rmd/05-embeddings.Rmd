...
We present visual diagnostics for the pragmatic usage of NLDR methods
...
structure and through user interactions like linked brushing observe
where the NLDR view may be misleading. We display several case studies from
both simulated and real data from single cell transcriptomics, that shows our
approach is useful for cluster orientation tasks.  The implementation of our 
framework is available as an R package called **liminal** available at
...
animated visualization [@Asimov1985-vp; @Buja1986-zr]. Given the dynamic nature
of the tour, user interaction is  important for controlling and exploring the 
visualisation: the tour has been used previously by @Wickham2015-cx and  
...
to interact with these techniques: when is an embedding suffucient for the
...
toolkits, **XGobi** and **GGobi** into a new tool called **GGvis** 
...
@Ovchinnikova2020-sy created the **Sleepwalk**  interface for checking
...
Then we describe the visual design of **liminal**, followed by implementation
...
**liminal** and plans for extensions to the software.
...
**liminal** is able to  visualise guided tours, our focus in the case studies 
...
**liminal**.  Our interface allows analysts to quickly compare 
...
linked views with the tour enchances interaction techniques, and allows 
...
pause the tour path, allowing users to identify 'interesting' projections. The
domain of  the axis scales from running a tour is called the half range, and is 
computed by rescaling the input data onto hyper-dimensional unit cube. We bind 
...
view as has been done in previous tour interfaces **XGobi** and **DataViwer** 
...
**liminal** [@r-liminal]. The package allows analysts to construct concatenated
visualisations, drawn with the **Vega-Lite** grammar of interactive graphics via
the **vegawidget** package [@Satyanarayan2017-gs; @Lyttle2020-hp]. It provides 
an interface for constructing linked and stand alone interfaces for 
manipulating tour paths via  the **shiny** and **tourr** packages 
...
The interface is constructed as a **shiny** gadget specifically designed for
...
The first three case studies qw use simulations where cluster structure and
geometery of the underlying data is known. We start with a simple example
...
The videos presented show the visual 
appearance of the **liminal** interface, and how we can interact with the tour
via the controls previously described. Both code and videos available for 
reproducing this paper are available at https://github.com/sa-lee/paper-liminal.
...
(ref:liminal-gaussian) Screenshots of the **liminal** interface applied to well clustered data, a video of the tour animation is available at https://player.vimeo.com/video/439635921. *(a)*: 

```{r gaussian, fig.cap = "(ref:liminal-gaussian)"}
```
...
We see in figure \@ref(fig:gaussian) and linked video that t-SNE has 
...
This can be confirmed via brushing the t-SNE view (figure \@ref(fig:gaussian)b). 


### Case Study 2: Exploring spherical Gaussian clusters with hierarchical structure

We repeat the process of viewing Gaussian clusters from the 
...
far from each other in the tour view (figure \@ref(fig:hierachical)a). We see 
in this case that is difficult
to see the sub-clusters in the tour view, however, once we zoom and highlight
they become more apparent (figure \@ref(fig:hierachical)b). When we brush 
...
(ref:liminal-hierachical) Screenshots of the **liminal** interface applied to subclustered data, a video of the tour animation is .available at https://player.vimeo.com/video/43963590 

```{r hierachical, fig.cap="(ref:liminal-hierachical)"}
```
...
tour with t-SNE layout with **liminal** as shown in figure \@ref(fig:trees-01).

(ref:trees-01) Screenshots of the **liminal** interface applied to tree structured data, a video of the tour animation is available at https://player.vimeo.com/video/439635892.   

```{r trees-01, fig.cap = "(ref:trees-01)"}
```
...
We see that the t-SNE view has been unable to recapitulate the
topology of the tree - the backbone (blue) branch has been split into three
fragments (figure \@ref(fig:trees-01)a). We can see this immediately via the linked highlighting over both
...
these results with our tour view as displayed in figure \@ref(fig:trees-02).


(ref:trees-02) Screenshots of the **liminal** interface applied to tree structured data, a video of the tour animation is available at https://player.vimeo.com/video/439635863.  
...
Figure \@(ref:trees-02) shows that this selection of parameters results in the 
...
get a sense of the shape of the clusters as seen in figure \@ref(fig:mouse-01).


(ref:mouse-01) Screenshots of the **liminal** interface applied to single cell data. A video of the tour animation is available at https://player.vimeo.com/video/439635812. 

```{r mouse-01, fig.cap = "(ref:mouse-01)" }
```
...
We observe that the embedding has mostly captured the
...
log normalised expression values using the same subset as before figure \@ref(fig:mouse-02).

(ref:mouse-01) Screenshots of the **liminal** tour applied to a marker gene set. A video of the tour animation is available at https://player.vimeo.com/video/439635843.


```{r mouse-02, fig.cap = "(ref:mouse-02)"}
```
...
We see that the expression of the marker genes, appear to separate
