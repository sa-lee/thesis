...
reads that overlap a single position of the reference genome, and is important for assessing sequencing data quality and used in many different aspects of omic's analysis such peak-calling in ChIP-seq or varinat calling in DNA-seq [@Sims2014-qk]. Here we emphasise looking at coverage traces to find biological events of interest, rather than only relying on numerical summaries of the data. By facetting these traces over combinations of the experimental design and with their biological context such as gene annotations, we can gain an
...
be possible with a single command line tool. There are also gains in reproduciblity as analysts do not have to move between multiple software suites to explore their processed transcriptomics data. 
...
(ref:si-fig): An overview of the **superintronic** and **plyranges** workflow.

```{r si-fig, fig.cap = "(ref:si-fig)", echo = FALSE}
knitr::include_graphics(here::here("img", "si-wflow.png"))
```
...
can be coereced to **GRanges** objects, are  incorporated into the coverage **GRanges** by taking the intersection of
...
provides some syntatic sugar for unravelling gene annotations into their exonic
...
the descriptors can be visualised directly as a scatterplot matrix. After that
