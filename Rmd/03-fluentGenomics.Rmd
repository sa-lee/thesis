...
The result is a _SummarizedExperiment_ object.
...
code chunks for the remainder of this section are not run.
...
We have already used **plyrange**  a number of times above, to `filter()`,
...
Similarly, we can then combine the `boot_genes` _GRanges_, with the DE
...
For all variables except for the _origin_ column we divide the first row's
