...
genomic algebra, as **dply**` is to the relational algebra.  The
...
\begin{tabular}{|l|l|p{6cm}|}
  ...
  \hline
   ...
   Aggregate & \textt{disjoin\_ranges()} & aggregate column(s) over the union of end coordinates \\
   &  \textt{reduce\_ranges()} & aggregate column(s) by merging overlapping and neighboring ranges \\
   \hline
   &  \textbf{\textt{mutate()}} & modifies any column \\
   & \textbf{\textt{select()}} & select columns \\
  Modify (Unary) & \textbf{\textt{arrange()}} & sort by columns \\
   & \textt{stretch()} & extend range by fixed amount \\
   &  \textt{shift\_(direction)} & shift coordinates \\
   & \textt{flank\_(direction)} & generate flanking regions \\
   & \textt{\%intersection\% } & row-wise intersection \\
   & \textt{\%union\%} & row-wise union \\
   & \textt{compute\_coverage} & coverage over all ranges \\
  Modify (Binary) &  \textt{\%setdiff\%} & row-wise set difference \\
   & \textt{between()} & row-wise gap range \\
   & \textt{span()} & row-wise spanning range \\
   \hline
    & \textt{join\_overlap\_*()} & merge by overlapping ranges \\
    & \textt{join\_nearest} & merge by nearest neighbor ranges \\
    & \textt{join\_follow} & merge by following ranges \\
    Merge & \textt{join\_precedes} & merge by preceding ranges \\
    & \textt{union\_ranges} & range-wise union \\
    & \textt{intersect\_ranges} & range-wise intersect \\
    & \textt{setdiff\_ranges} & range-wise set difference \\
    & \textt{complement\_ranges} & range-wise set complement \\
  \hline
   & \textt{anchor\_direction()} & fix coordinates at direction \\
  Operate & \textbf{\textt{group\_by()}} & partition by column(s)  \\ 
   & \textt{group\_by\_overlaps()} & partition by overlaps \\
   \hline
   & \textbf{\textt{filter()}} & subset rows \\
  Restrict & \textt{filter\_by\_overlaps()} & subset by overlap \\
    & \textt{filter\_by\_non\_overlaps()} & subset by no overlap \\
   \hline
\end{tabular}
