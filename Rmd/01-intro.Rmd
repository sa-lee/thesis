...
statistical modelling (figures \@ref(fig:wflow-cycle)). As datasets grow in complexity and become
...
(ref:wflow-integrate): Data from multiple assays can be combined together using modelling and transformations. Data and model results can combined, tidied and transformed to have a common representation across different granularities of the genome.

(ref:wflow-integrate): Visualisation is a tool for making sense of models. Here we explore, how well a model has captured the structure within the data using interactive graphics.  
...
and distortions obtained from an NLDR method. This approach acknowledges that
...
distortions. We present several case studies using data sets  both 
single-cell transcriptomics and  machine learning  for using our approach to diagnose the quality   results obtained via popular NLDR methods
like t-distributed stochastic neighbour embeddings (t-SNE) [@Maaten2008-sk].
