...
according to the aforementioned workflow steps. To begin we discuss wrangling
biological data using a coherent represntation and programming interface; 
section \@ref(sec:tidygranges)
introduces a grammar-based framework for transforming genomics data that is 
described in chapter \@ref(ch:plyranges). Next, we look at integrating data and 
model outputs over genomic regions to gain biological insight; \@ref(sec:ir) 
...
\@ref(ch:intron). Third, we consider the challenges in visualising  
high dimensional data; section \@ref(sec:va) introduces a visual analytics 
approach for understanding non-linear dimension reduction techniques described
in chapter \@ref(ch:tsne). Lastly, in chapter 
...
together. This chapter has been published in @Lee2020-ie. 
...
signal from RNA-seq data. This chapter is based on the preprint @Lee2019-mf. 
...
case studies using data sets from both single-cell transciptomics and 
machine learning  for using our approach to diagnose the quality of 
results obtained via popular non-linear DR methods
