...
on applications to genomics and transcriptomics. It is organised in 
the same manner that you would iterate back and
forth between these tasks during an analysis. To begin you wrangle your data 
into a structure that you can understand and use; section \@ref(sec:tidygranges)
introduces a grammar-based framework for transforming genomics data that is 
described in chapter \@ref(ch:plyranges). Next, you integrate data and model 
outputs over genomic regions to gain biological insight; \@ref(sec:ir) 
introduces a framework for incorporating and discovering 'interesting' genomic
regions, described further in chapters \@ref(ch:fluentGenomics) and 
\@ref(ch:intron). Third, you cycle between model-based representations and 
visualisations of your data; section \@ref(sec:va) introduces a visual analytics 
approach for understanding non-linear dimension reduction techniques and is 
described in detail in chapter \@ref(ch:tsne). Lastly, you take a step back and 
look at the implications and limitations of your work; in chapter 
\@ref(ch:conclusion) I describe the outputs of my thesis and plans for future 
research. 
...
## Integration and representation of genomic data structures {#sec:ir}
...
we describe a simpler end-to-end workflow for integrating 
...
Moreover, in chapter \@ref(ch:intron) we explore the limits of the *tidy data*
...
signal from RNA-seq data.


## Visual analytics for dimension reduction workflows {#sec:va}
...
transcriptomics): dimension reduction (DR). We consider the incorporation of 
interactive and dynamic graphics to assist analysts in using DR techniques
for cluster orientation tasks. In particular, we advocate for the use of tours
[@Cook1995-bi] alongside a DR visualisation to highlight potential pitfalls and 
distortions of obtained from a DR method. This approach acknowledges that
there is no 'one' best visualisation or DR for a high-dimensional dataset,
and we often want to have an understanding of both the global and local structure
within our data.


Chapter \@ref(ch:liminal) introduces
a software package called *liminal* for constructing these views and
a user interaction framework for identifying distortions. We present several
case studies for using our approach to diagnose the quality of 
results obtained via t-distributed stochastic neighbour embeddings (t-SNE)
